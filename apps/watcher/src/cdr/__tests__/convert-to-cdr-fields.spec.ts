...
import { convertToCDRFields, CDRLine } from '../convert-to-cdr-fields';
...
describe('convertToCDRFields', () => {
  ...
  beforeEach(() => {
    vi.resetAllMocks();
    (uuidV4 as vi.Mock).mockReturnValue(mockUuid);
    (getDateFromFormatted as vi.Mock).mockReturnValue(mockDate);
  });

  it('should correctly process a valid CDR line', () => {
    (validateCdrType as vi.Mock).mockReturnValue('voice'); // Assuming 'voice' is a valid CDRType
    const cdrLineParts = [
      'voice', // recordType
  ...
  it('should handle invalid recordType when validateCdrType throws an error', () => {
    ...
    (validateCdrType as vi.Mock).mockImplementation(() => {
      throw error;
    });
    ...
    expect(result.timestamp).toEqual(mockDate); // timestamp is processed
  ...
  it('should handle edge cases for numeric parsing (missing or malformed values)', () => {
    (validateCdrType as vi.Mock).mockReturnValue('sms'); // Assuming 'sms' is a valid CDRType
  ...
  it('should handle optional fields being undefined (not present in array)', () => {
    (validateCdrType as vi.Mock).mockReturnValue('data'); // Assuming 'data' is a valid CDRType
    const cdrLineParts = [
      'data', // recordType
      '20231026120000', // timestamp
      '500', // eventDuration
